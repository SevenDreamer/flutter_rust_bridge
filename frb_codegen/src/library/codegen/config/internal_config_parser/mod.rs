...
impl InternalConfig {
    pub(crate) fn parse(config: &Config, meta_config: &MetaConfig) -> Result<Self> {
        let base_dir = match config.base_dir.as_ref().map(dunce::canonicalize) {
            Some(Ok(path)) => path,
            None | Some(Err(_)) => dunce::canonicalize(std::env::current_dir().expect("failed to get current dir"))?,
        };
